...
## **HARDWARE**
На данный момент в устройстве установлено 3 нагревательных элемента с общей мощностью около 70Вт. Каждый элемент имеет собственную плату управления (силовой ключ) на базе MOSFET транзистора D4184 и цифровой датчик контроля температуры DS18В20. Для работы датчиков температуры используются библиотеки "OneWire.h" и "DallasTemperature.h"
...
Подключение происходит по сети Wi-Fi, где в качестве точки доступа выступает ESP32. Для работы с функциями Wi-Fi используется библиотека <WIFI.h>
Работа с пользователем организованна с помощью асинхронного TCP Web-сервера, для которого использованны библиотеки <ESPAsyncWebServer.h> и <AsyncTCP.h>. 
...
Дополнительно для упрощенного подключения по имени сети вместо IP адреса применяется DNS сервер из библиотеки <DNSServer.h>
